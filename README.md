...
**Por qué se agregó JMX (`JMX_PORT`, `KAFKA_CLUSTERS_0_METRICS_PORT`):** Kafka-UI puede mostrar tópicos y mensajes sin esto (es protocolo normal de Kafka), pero su pantalla de **Dashboard** (gráficos de actividad: bytes in/out por segundo, etc.) depende específicamente de métricas JMX — sin exponerlas, esos gráficos quedan vacíos aunque todo lo demás funcione. Se expuso el puerto JMX del broker (`JMX_PORT: 9101`) y se le indicó a Kafka-UI dónde encontrarlo (`KAFKA_CLUSTERS_0_METRICS_PORT: 9101`). Se verificó con `curl http://localhost:8080/api/clusters`: `bytesInPerSec` pasó de `null` (sin métricas) a un valor real que se mueve al mandar mensajes.
