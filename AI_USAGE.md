...
9. **Patrón estricto de `vehicleId` expuso un bug preexistente de "tomar el primer mensaje que llegue".** El usuario pidió reforzar el contrato de `vehicleId` a un patrón estricto (`^[A-Z]{3}-[0-9]{3}$`), lo que obligó a cambiar el ejemplo oficial del PDF (`VEH-99`, 2 dígitos) a `VEH-099` en el test. Al correrlo, el test falló por integridad — no por el patrón, sino porque el consumer, con `auto_offset_reset="earliest"` + `break` en el primer mensaje recibido, estaba leyendo el mensaje **más viejo del tópico** (offset 0, acumulado de corridas de sesiones anteriores), no el que se acababa de publicar. Antes pasaba inadvertido porque, por coincidencia, ese mensaje viejo también decía "VEH-99" — el cambio de valor lo expuso. Se corrigió haciendo que el consumer siga leyendo hasta encontrar exactamente la partición+offset que el broker confirmó al productor (`record_metadata`), en vez de conformarse con "el primero que llegue". Lección: un test que "pasa" puede estar acertando por casualidad, no por diseño correcto — cambiar un dato de prueba, aunque parezca trivial, puede desenmascarar eso.

10. **Kafka-UI no se conectaba al broker con la configuración de listener existente.** El `docker-compose.yml` original solo anunciaba `PLAINTEXT://localhost:9092`, pensado para clientes fuera de Docker (los scripts Python en Windows). Kafka-UI corre como otro contenedor en la misma red de Docker; si se conectaba y el broker le anunciaba "localhost", el contenedor de Kafka-UI entendería eso como "yo mismo" y no encontraría el broker — un problema clásico y bien documentado de Kafka en Docker (listeners internos vs. externos), no una casualidad de este proyecto. Se agregó un segundo listener (`INTERNAL://kafka:29092`, usando el nombre del servicio de Compose) dedicado a clientes intra-Docker, sin modificar el listener original — verificado con `curl` contra la API HTTP de Kafka-UI (`/api/clusters`) confirmando `"status":"online"` y el tópico real visible, y re-corriendo el test oficial para confirmar que los scripts Python en Windows no se vieron afectados. Efecto colateral aceptado: recrear el contenedor del broker (necesario para aplicar el nuevo listener) borró los mensajes acumulados de sesiones anteriores, al no haber volumen persistente configurado — comportamiento esperado de un entorno de desarrollo, documentado en el README.

11. **Dashboard de Kafka-UI vacío pese a que Topics/Messages sí funcionaban.** El usuario reportó que la pantalla principal de Kafka-UI no mostraba nada dinámico. Se diagnosticó con `curl http://localhost:8080/api/clusters`: `bytesInPerSec`/`bytesOutPerSec` venían `null` — esas métricas de throughput dependen de JMX, un mecanismo separado del protocolo normal de Kafka (que ya funcionaba, por eso Topics/Messages sí se veían). Se expuso `JMX_PORT` en el broker y se configuró `KAFKA_CLUSTERS_0_METRICS_PORT` en Kafka-UI; se reconfirmó con el mismo `curl` que las métricas pasaron de `null` a valores reales que se mueven al enviar mensajes, y que el test oficial y los scripts seguían funcionando sin cambios.
    - **Alucinación propia corregida en el mismo hilo:** se afirmó inicialmente que el Dashboard mostraría "gráficos de actividad". El usuario mandó una captura real mostrando que solo aparecen números (Production/Consumption en bytes), no gráficas de línea — esta versión de Kafka-UI (v0.7.2) no las dibuja. Se corrigió la afirmación en el README y se le presentó al usuario la opción real (Grafana + Prometheus) para tener gráficas de verdad, dejándolo decidir; eligió no agregar más piezas al stack por mantenerlo liviano.

En los once casos, la corrección no fue "la IA se equivocó y ya" — fue el resultado de verificar contra el sistema real (correr el build, correr el test —varias veces, no solo una—, hacer la petición HTTP real, comprobar que un archivo de instalación efectivamente existe) en vez de asumir que algo funcionó por haberse reportado como exitoso o por haber pasado una sola vez.
